...
class Fuzzer:
    ...
    def execute_binary(self) -> str:
        ...
        try:
            ...
            result = subprocess.run(['java', '-cp', testdir, "test", *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=2)
        ...
        if len(stderr) != 0:
            ...
            if "Invalid pc" in err[1] or "Control flow" in err[1]:
                ...
                if err[4].strip().split()[-1] == "<illegal>" or err[4].strip().split()[-1][:5] == "fast_":
                    return "Error"
                else:
                    return "Bad"
            else:
    ...
    def generate(self, min_len: int, max_err: int, start: str, end: str, code_attr_len: int, max_lcls: int) -> str:
        """
    ...
    def get_valid(self, num: int, min_len: int, max_err: int) -> list:
        ...
        start_idx, end_idx, code_attr_length, max_lcls = self.main_finder("../template/test.class")
        f = open("../template/test.class", "rb")
        ...
        while len(valid) < num:
            ...
            valid.append(self.generate(min_len, max_err, start, end, code_attr_length, max_lcls))
            self.num_created += 1
