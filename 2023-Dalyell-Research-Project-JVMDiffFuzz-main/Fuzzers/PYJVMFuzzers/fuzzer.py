...
class Fuzzer:
    ...
    def execute_binary(self) -> str:
        ...
        """
        try:
            result = subprocess.run(['python3', "run_unittest.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=2)
    ...
    def generate(self, min_len: int, max_err: int, start: str, end: str, code_attr_len: int) -> str:
        """
    ...
    def get_valid(self, num: int, min_len: int, max_err: int) -> list:
        ...
        valid = list()
        while len(valid) < num:
            valid.append(self.generate(min_len, max_err, start, end, code_attr_length))
